...
void loop() {
  ...
  if (packetSize) {
    ...
    int id_float_start = LoRaData.indexOf("hiveID:") + String("hiveID:").length(); 
    if (hum_float_start > 0) {
      ...
      String id_float_string = LoRaData.substring(id_float_start);
      float received_id = id_float_string.toFloat();
      Serial.print(" -> hiveID: ");
...
String postData = "temperature=" + String(temperature) + "&humidity=" + String(humidity_value) + "&hiveID=" + String(hiveID);
  HTTPClient http;
  http.begin(URL);
